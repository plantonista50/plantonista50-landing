...
export function FinalCta() {
  return (
    <section className="final relative overflow-hidden py-32 md:py-44 px-5 text-center" id="cta">
      <span className="absolute top-0 left-1/2 -translate-x-1/2 rounded-full pointer-events-none" aria-hidden="true" style={{ width: 1400, height: 700, background: "radial-gradient(ellipse at center, var(--accent-glow), transparent 65%)", opacity: 0.4 }} />
      <span className="absolute -bottom-24 left-1/2 -translate-x-1/2 rounded-full pointer-events-none" aria-hidden="true" style={{ width: 1000, height: 400, background: "radial-gradient(ellipse at center, var(--accent-tint), transparent 65%)" }} />
      <div className="relative max-w-[900px] mx-auto" data-reveal data-dir="up">
        ...
        <a href="/" data-magnetic className="btn-primary group relative overflow-hidden bg-ink text-[#101114] py-4 md:py-[18px] px-7 md:px-8 rounded-[10px] font-bold inline-flex items-center justify-center gap-2.5 animate-breathe transition-shadow duration-500 ease-power3" style={{ fontSize: "1.04rem", letterSpacing: "-.005em", willChange: "transform" }}>
          <span className="relative z-[1]">Entrar no Plantão</span>
        ...
        <div className="flex justify-center gap-5 flex-wrap mt-7 font-mono">
          {["Sem cartão", "10 análises grátis/mês", "Exclusivo para médicos", "A IA nunca vê o nome"].map((t) => (
            <span key={t} className="inline-flex items-center gap-2 text-[.84rem] text-ink-2">
              <Icon name="check_circle" size={16} className="text-accent" />{t}
            </span>
          ))}
        </div>
      </div>
